...
def drawSegment(baseImg, matImg, name):
...
def run_visualization():
  ...
  for i in range(1, 51):
    ...
    img ="D:/Daiva/Research/Main/41. Background Removal/1. Background Removal/rgb/"+name+".jpg"
    
    """Inferences DeepLab model and visualizes result."""
    try:
      print("Trying to open : " + img)
      # f = open(sys.argv[1])
      jpeg_str = open(img, "rb").read()
      orignal_im = Image.open(BytesIO(jpeg_str))
    except IOError:
      print('Cannot retrieve image. Please check file: ' + img)
      return
    ...
    drawSegment(resized_im, seg_map, name) 
